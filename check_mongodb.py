...
def numeric_type(param):
    return type(param) == float or type(param) == int or type(param) == int or param is None
...
def main(argv):
    ...
    p.add_option('-P', '--port', action='store', type='int', dest='port', default=27017,
    ...
    p.add_option('-A', '--action', action='store', type='choice', dest='action', default='connect',
                 help='The action you want to take',
                 choices=['connect', 'connections', 'replication_lag', 'replication_lag_percent', 'replset_state',
                          'memory', 'memory_mapped', 'lock',
                          'flushing', 'last_flush_time', 'index_miss_ratio', 'databases', 'collections',
                          'database_size', 'database_indexes', 'collection_indexes', 'collection_size',
                          'collection_storageSize', 'queues', 'oplog', 'journal_commits_in_wl', 'write_data_files',
                          'journaled', 'opcounters', 'current_lock', 'replica_primary',
    ...
    p.add_option('--max-lag', action='store_true', dest='max_lag', default=False,
                 help='Get max replication lag (for replication_lag action only)')
    p.add_option('--mapped-memory', action='store_true', dest='mapped_memory', default=False,
                 help='Get mapped memory instead of resident (if resident memory can not be read)')
    p.add_option('-D', '--perf-data', action='store_true', dest='perf_data', default=False,
                 help='Enable output of Nagios performance data')
    p.add_option('-d', '--database', action='store', dest='database', default='admin',
                 help='Specify the database to check')
    p.add_option('--all-databases', action='store_true', dest='all_databases', default=False,
                 help='Check all databases (action database_size)')
    p.add_option('-s', '--ssl', dest='ssl', default=False, action='callback', callback=optional_arg(True),
                 help='Connect using SSL')
    p.add_option('-r', '--replicaset', dest='replicaset', default=None, action='callback', callback=optional_arg(True),
                 help='Connect to replicaset')
    p.add_option('-q', '--querytype', action='store', dest='query_type', default='query',
                 help='The query type to check [query|insert|update|delete|getmore|command] from queries_per_second')
    p.add_option('-c', '--collection', action='store', dest='collection', default='admin',
                 help='Specify the collection to check')
    p.add_option('-T', '--time', action='store', type='int', dest='sample_time', default=1,
                 help='Time used to sample number of pages faults')
    p.add_option('-M', '--mongoversion', action='store', type='choice', dest='mongo_version', default='2',
                 help='The MongoDB version you are talking with, either 2 or 3',
                 choices=['2', '3'])
    p.add_option('-a', '--authdb', action='store', type='string', dest='authdb', default='admin',
    ...
    try:
        ...
        if pymongo.version >= "2.3":
            if replica is None:
                ...
                con = pymongo.MongoClient(host, port, read_preference=pymongo.ReadPreference.SECONDARY, ssl=ssl,
                                          replicaSet=replica)
        else:
            if replica is None:
...
def check_rep_lag(con, host, port, warning, critical, percent, perf_data, max_lag, user, passwd):
    ...
    try:
        ...
        try:
            ...
            if ((e.code is None and str(e).find('failed: not running with --replSet"')) or (
                            e.code == 76 and str(e).find('not running with --replSet"'))):
        ...
        if serverVersion >= tuple("2.0.0".split(".")):
                ...
                if not max_lag:
                    ...
                    if percent:
                        err, con = mongo_connect(primary_node['name'].split(':')[0],
                                                 int(primary_node['name'].split(':')[1]), False, user, passwd)
                        if err != 0:
                            return err
                        ...
                        message += performance_data(perf_data,
                                                    [(maximal_lag, "replication_lag_percent", warning, critical)])
                    else:
            ...
            return check_levels(lag, warning + slaveDelays[host_node['name']],
                                critical + slaveDelays[host_node['name']], message)
        else:
            #
            # less than 2.0 check
            #
...
def check_flushing(con, warning, critical, avg, perf_data):
    ...
    try:
        ...
        try:
            ...
            message += performance_data(perf_data, [
                ("%.2fms" % flush_time, "%s_flush_time" % stat_type.lower(), warning, critical)])
...
def check_queues(con, warning, critical, perf_data):
    ...
    try:
        ...
        message += performance_data(perf_data, [(total_queues, "total_queues", warning, critical),
                                                (readers_queues, "readers_queues"), (writers_queues, "writers_queues")])
...
def check_queries_per_second(con, query_type, warning, critical, perf_data, mongo_version):
    ...
    try:
        ...
        try:
            ...
            if mongo_version == 2:
                db.nagios_check.update({'_id': last_count['_id']},
                                       {'$set': {"data.%s" % query_type: {'count': num, 'ts': int(time.time())}}})
            else:
                db.nagios_check.update_one({'_id': last_count['_id']},
                                           {'$set': {"data.%s" % query_type: {'count': num, 'ts': int(time.time())}}})

            message = "Queries / Sec: %f" % query_per_sec
            message += performance_data(perf_data,
                                        [(query_per_sec, "%s_per_sec" % query_type, warning, critical, message)])
        except KeyError:
            #
            ...
            if mongo_version == 2:
                db.nagios_check.insert(
                    {'check': 'query_counts', 'data': {query_type: {'count': num, 'ts': int(time.time())}}})
            else:
...
def check_oplog(con, warning, critical, perf_data):
    ...
    try:
        ...
        if db.system.namespaces.find_one({"name": "local.oplog.rs"}) is not None:
            oplog = "oplog.rs"
        else:
            ol = db.system.namespaces.find_one({"name": "local.oplog.$main"})
            if db.system.namespaces.find_one({"name": "local.oplog.$main"}) is not None:
                oplog = "oplog.$main"
            else:
        ...
        message += performance_data(perf_data, [("%.2f" % hours_in_oplog, 'oplog_time', warning, critical),
...
def check_opcounters(con, host, warning, critical, perf_data):
    ...
    if err1 == 0 and err2 == 0:
        ...
        message = "Opcounters: total=%d,insert=%d,query=%d,update=%d,delete=%d,getmore=%d,command=%d" % tuple(
            per_minute_delta)
        message += performance_data(perf_data, (
            [(per_minute_delta[0], "total", warning, critical), (per_minute_delta[1], "insert"),
...
def check_replica_primary(con, host, warning, critical, perf_data, replicaset, mongo_version):
    ...
    if current_primary != saved_primary:
        ...
        if mongo_version == 2:
            ...
            db.last_primary_server.update_one({"_id": "last_primary"}, {"$set": last_primary_server_record},
                                              upsert=True)
...
def check_page_faults(con, sample_time, warning, critical, perf_data):
    ...
    try:
        ...
        try:
            ...
            page_faults = (int(data2['extra_info']['page_faults']) - int(
                data1['extra_info']['page_faults'])) / sample_time
        except KeyError:
